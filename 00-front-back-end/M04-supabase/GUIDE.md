...
> 대시보드(Table Editor)에서 테이블을 만들면 **RLS(Row Level Security, 행 단위 보안)가 기본값으로 켜진 채** 생성된다. 즉 Firebase의 "테스트 모드"처럼 처음에 모두 허용되는 게 아니라, **정책(Policy)을 추가하기 전까지는 기본적으로 아무도 데이터를 읽거나 쓸 수 없다.** 이 문서의 3단계에서 이 부분을 반드시 짚고 넘어간다.
