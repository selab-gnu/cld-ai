...
**여기서 중요한 포인트:** 코드는 1번 → 2번 → 3번 순서로 "적혀" 있지만, 실행 결과는 1번 → 3번 → 2번입니다. `setTimeout`이 "나는 1초 뒤에 실행할 테니, 너(Node.js)는 기다리지 말고 다음 줄(3번)을 먼저 실행해"라고 동작하기 때문입니다. 이게 비동기의 핵심입니다: **시간이 걸리는 작업은 뒤로 미뤄두고, 그 사이에 다른 코드를 먼저 실행한다.**
